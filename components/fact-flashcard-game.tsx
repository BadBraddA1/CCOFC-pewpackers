...
export default function FactFlashcardGame({ data }: { data: FactItem[] }) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [isFlipped, setIsFlipped] = useState(false)
  const [shuffledData, setShuffledData] = useState<FactItem[]>([])
      ...
      <Card className="w-full aspect-[3/2] mb-6 hover:shadow-lg transition-shadow duration-300">
        ...
        >
          <div className="text-center">
            ...
            {isFlipped ? (
              <p className="text-xl sm:text-2xl">{shuffledData[currentIndex].answer}</p>
            ) : (
              <h3 className="text-2xl sm:text-3xl font-bold">{shuffledData[currentIndex].question}</h3>
            )}
          </div>
        </CardContent>
      </Card>
